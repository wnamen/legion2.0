  ...
  render(){
    ...
    if (data !== undefined) {
      cadences = data.map((cadence, index) => {
        return (
          <CampaignListing key={index} campaignID={cadence.id} campaignName={cadence.name} campaignStatus={cadence.status} renderCampaign={this.props.renderCampaign} deleteCampaign={this.props.deleteCampaign}/>
        )
      })
    }
