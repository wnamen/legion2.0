    ...
    if (templates !== null) {
      mappedTemplates = templates.map((template, index) => {
        if ((this.props.renderState === "campaign") && (template !== undefined)) {
          return (
            <Template key={`template ${template.id || index}`} data={template} dataIndex={index} templateData={this.props.templateData} handleTemplateSave={this.handleTemplateSave} currentDelay={delays[index]} onDelayChange={this.onDelayChange} currentTemplates={this.state.currentTemplates || this.props.currentTemplates} currentDelays={this.state.currentDelays || this.props.currentDelays} onAppendTemplate={this.onAppendTemplate} sendTestEmail={this.props.sendTestEmail} copiedToClipBoard={this.props.copiedToClipBoard}/>
          )
        } else if (template !== undefined) {
          return (
            <Template key={`template ${template.id || index}`} data={template} dataIndex={index} templateData={this.props.templateData} handleTemplateSave={this.handleTemplateSave} currentDelay={null} currentTemplates={this.state.currentTemplates || this.props.currentTemplates} currentDelays={this.state.currentDelays || this.props.currentDelays} onAppendTemplate={this.onAppendTemplate} sendTestEmail={this.props.sendTestEmail} copiedToClipBoard={this.props.copiedToClipBoard}/>
          )
        }
      })
    }
